<?php
    ...
    include('header.php'); //Call the php header.
		...
		$query = trim($_GET['search']);	 //Get search term and trim white space.
		

		
        //Returns results where the query matches the description of the product or the title or tags field. Case insensitive.
        $sql_query = "SELECT * FROM product WHERE UPPER(description) LIKE UPPER('%$query%') OR UPPER(name) LIKE UPPER('%$query%') OR UPPER(tags) LIKE UPPER('%$query%') ORDER BY price";
